<template>
  ...
  <main v-else class="mx-auto w-full max-w-lg p-4">
    <div class="flex min-h-full flex-col justify-center pb-8 sm:px-6 lg:px-8">
      <div class="sm:mx-auto sm:w-full sm:max-w-md">
        <img src="/img/icon.svg" class="mx-auto h-12 w-auto text-blue-500" alt="Send" />
      </div>
    </div>

    <header class="text-2xl font-semibold text-gray-800">
    ...
    <main class="mt-8">
      ...
      <div class="mt-12 rounded-sm border border-gray-200">
        <div class="p-4">
          <div class="flex h-full w-full items-center rounded-sm bg-white p-2">
            <svg
              class="h-12 w-auto text-blue-400"
            ...
            <div class="ml-4 leading-snug">
              <span id="filename" class="text-gray-700">{{ meta?.name || '' }}</span
              ><br />
      ...
      <div class="mt-12">
        <loadable-button
          :loading="downloading"
          class="w-full rounded-sm bg-blue-700 py-3 text-center font-semibold text-gray-100"
          type="button"
          @click="downloadFile"
        >
          Download
...
export default defineComponent({
  ...
  async created() {
    ...
    const signingKey = await filenc.deriveHmacSigningKey(baseKey, Uint32Array.from(attempt.signingKey.salt), false)

    const signature = await crypto.subtle.sign({ name: 'HMAC' }, signingKey, new TextEncoder().encode(attempt.nonce))
  ...
  methods: {
    ...
    async downloadFile() {
      const fileKey = await filenc.deriveAesGcm256Key(<CryptoKey>this.baseKey, Uint32Array.from(this.file.salt))
      ...
      try {
        ...
        notifier().warning("We couldn't download your file", 'Please try again in a few minutes.')
      }
